---
name: Daily Brief
description: 每日阅读简报：根据用户兴趣，从互联网收集最新新闻，生成供用户个人阅读的新闻摘要。
allowed-tools: [search_web, bash, read_url_content, view_file, write_to_file]
---

# 每日阅读简报 (Daily Brief)

## 流程:

### 1. 兴趣发现
分析用户工作区以识别兴趣，并**生成多样化的搜索查询**。
- 调用 `interest-analyzer` Agent。
- **输出重点**：必须关注其生成的 "🔍 搜索配置" 部分，包括新闻、教程、工具和社区讨论。

### 2. 内容收集 (执行搜索与深度阅读)
这里的执行必须严格基于第 1 步的输出：
- 读取 `interest-analyzer` 输出中的搜索查询列表。
- **执行搜索 (Must be fresh)**：
    - **必须**使用 `after:YYYY-MM-DD` 过滤器 (设置为 7 天前)。
    - **严禁**搜索旧内容。
    - 使用 `search_web` 获取初步结果列表。
- **深度获取 (使用 Dev Browser)**：
    - 调用 `dev-browser` Skill (位于 `skills/dev-browser/`) 来访问关键链接。
    - **启动服务**：如果尚未运行，先执行 `./skills/dev-browser/server.sh &`。
    - **读取内容**：使用 Dev Browser 脚本访问页面并提取全文。
    - **优势**：如果有 Chrome 扩展连接，这可以复用用户的登录状态（如 Substack, Twitter）。
- **关键**：确保将**发布日期**和**全文内容**读入上下文。

### 3. 策划与简报 (News Curator)
调用 `news-curator` Agent 处理收集到的原始数据：
- **输入**：用户的兴趣画像 (来自 Step 1) + 原始搜索结果 (含日期)。
- **任务**：
    - 验证每条新闻的发布日期 (针对新闻，必须 < 7 天)。
    - 筛选高质量内容 (针对教程/资源，必须相关且近期)。
    - **生成上下文：解释为什么这条新闻对该特定用户重要**。
    - 建议可操作的后续步骤。

### 3. 简报生成
将策划的新闻格式化为每日简报：
- 按相关性和优先级分组
- **必须显示每条新闻的发布日期**
- 为每条信息添加 "Why it matters" (个人上下文)
- 建议可能采取的行动
- 突出显示紧急/有时效性的项目

### 4. 输出组织
将简报保存至 `daily-brief/YYYY-MM-DD-brief.md`，包含：
- 按主题划分的清晰章节
- **发布日期**
- **个人相关性上下文**
- 建议的行动
- 顶部的快速摘要
- **必须使用中文撰写**

## 主要功能:
- **个性化**：基于你文件中的真实兴趣
- **最新**：仅包含最近 7 天的新闻 (硬性限制)
- **已验证**：所有日期在包含前均已检查
- **可操作**：包含你可以用这些信息做什么
- **上下文化**：解释与你特定工作/兴趣的相关性

## 重要提示:
- **切勿**包含过时的新闻（超过 7 天）
- **始终**验证发布日期 (如果日期不清楚，则丢弃)

- **始终**使用带日期过滤的网络搜索
- **始终**解释个人相关性
- **专注于**可操作的信息
